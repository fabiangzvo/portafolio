...
const NavBar = (props) => {
  ...
  if (smallScreen) return <Container><GiHamburgerMenu onClick={() => setVisible(!visible)} />
    <StyledDrawer classes={{ paper: 'drawer-custom-class' }} anchor='left' elevation={32} open={visible} onClose={() => setVisible(!visible)}>
      ...
      <Menu flexDirection='column' justifyContent='space-around' height='30vh'>
        {
          constants.routes.map((route, index) => <li key={index}><Link fade duration={.7} top="entry" direction="rigth" to={route.path} onClick={(e) => console.log(e)}>{route.label}</Link></li>)
        }
      </Menu>
  ...
  return (
    <Nav bg='blue.0' className='navbar' flexDirection='column' justifyContent='space-around' alignItems='center'>
      ...
      <Menu flexDirection='column' justifyContent='space-around' height='30vh'>
        {
          constants.routes.map((route, index) => <Link fade duration={.7} top="entry" direction="rigth" key={index} to={route.path} onClick={(e) => { }}>{route.label}</Link>)
        }
      </Menu>
